...
- 重複制御：同じ4Hロールリバーサル（同一ブレイク足・同一方向）では、最初に成立した15Mエントリーだけ通知。以後は新しい4Hブレイクが出るまで再通知しない
- 押し目/戻り：15Mの直近4本がロールリバーサル水準へATR許容幅内で再接触し、最新15M足がトレンド方向へ確定
- ATR：ATR14を使用
- 損切り：4Hブレイク発生後の4H構造を基準化。LONGはブレイク後4H安値とロールリバーサル水準の低い方から4H ATR × 0.5下、SHORTはブレイク後4H高値とロールリバーサル水準の高い方から4H ATR × 0.5上
- 利確：ブレイク後の4H直近高値（SHORTは直近安値）を基準に、4H ATR × 0.25だけ内側へ調整
- 時間足の役割：15Mはエントリータイミング確認だけに使用し、SL/TPは4H構造と4H ATRで決定
...
Telegram通知時のEntryは条件成立した15分足の終値です。SL/TPは4H構造と4H ATRから決定し、15MはEntryタイミングだけに使用します。次の式で枚数を計算します。

```text
リスク予算 = 現在のEdgeX Equity × 5%
理論枚数 = リスク予算 ÷ |15M Entry - 4H構造SL|
