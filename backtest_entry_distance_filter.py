...
PERIOD_DAYS = 180
HOLDOUT_DAYS = 30
FILTERS = {"baseline": None, "atr_025": 0.25, "atr_050": 0.50, "atr_075": 0.75}
...
def backtest_contract(
    ...
    full_trades: dict[str, list[dict[str, Any]]] = {name: [] for name in FILTERS}
    pre_holdout_trades: dict[str, list[dict[str, Any]]] = {name: [] for name in FILTERS}
    monitor_end = 0
    seen_setup_keys: dict[str, set[str]] = {name: set() for name in FILTERS}
    ...
    for i, candidate in enumerate(entries):
        ...
        distance_atr = abs(candidate.close - signal.breakout_level) / float(signal.atr_monitor)
        for variant, threshold in FILTERS.items():
            if threshold is not None and distance_atr > threshold:
                continue
...
def main() -> None:
    ...
    for name, threshold in FILTERS.items():
        ...
        variants[name] = {
            "max_entry_distance_atr4h": threshold,
    ...
    result = {
        ...
        "hypothesis": "Keep 4H structure SL/TP fixed and reject 15M entries that close too far from the 4H roll level.",
        "filter_definition": "abs(15M entry close - 4H roll level) / 4H ATR",
        "filters": FILTERS,
        "variants": variants,
        "coverage": {
    ...
    summary_path = out_dir / "entry_distance_filter_summary.json"
    csv_path = out_dir / "entry_distance_filter_trades.csv"
