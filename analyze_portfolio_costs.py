...
SRC=Path("backtest_results/deduped_180d_trades.csv")
OUT=Path("backtest_results/portfolio_cost_risk_analysis.json")
START=200.0
...
def load():
    ...
    with SRC.open(encoding="utf-8",newline="") as f:
        for x in csv.DictReader(f):
            ...
            rows.append({
              "symbol":x["symbol"],"entry_t":ts(x["entry_time_utc"]),"exit_t":ts(x["exit_time_utc"]),
...
rows=load()
raw_net=sum(x["r"] for x in rows)
stop_pcts=sorted(x["stop_pct"] for x in rows)
def pct(v,p):
    if not v:return None
    k=(len(v)-1)*p; a=math.floor(k); b=math.ceil(k)
    return v[a] if a==b else v[a]*(b-k)+v[b]*(k-a)
result={
 "source_trades":len(rows),"starting_equity_usdc":START,
 ...
 "important_limits":[
   "Funding is not included: the source trade file has no historical funding cash flows.",
   "Slippage scenarios are fixed percentage stress tests, not order-book replay.",
   "Portfolio risk cap treats each open trade as its full initial stop risk until exit; it does not reduce risk after partial profit.",
   "One position per symbol is enforced.",
   "Stock-perpetual market-hours execution constraints are not modeled in this derived trade-file analysis."
 ],
 "cost_scenarios":{},
}
for name,c in COSTS.items():
    costs=[cost_r(t,c) for t in rows]
    block={
      "assumption":c,
      "cost_r":{"mean":sum(costs)/len(costs),"median":pct(sorted(costs),.5),"p90":pct(sorted(costs),.9)},
      "no_portfolio_filter":{
        "net_r_after_cost":sum(t["r"]-cost_r(t,c) for t in rows),
        "avg_r_after_cost":sum(t["r"]-cost_r(t,c) for t in rows)/len(rows)
      },
      "portfolio_grid":[]
    }
    for risk in RISKS:
      for cap in CAPS:
        if risk<=cap:
          block["portfolio_grid"].append(simulate(rows,risk,cap,c,True))
    result["cost_scenarios"][name]=block
