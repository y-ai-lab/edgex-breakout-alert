...
PERIOD_DAYS = 180
HOLDOUT_DAYS = 30
VARIANTS = {"baseline": None, "body_atr_050": 0.50, "body_atr_075": 0.75, "body_atr_100": 1.00}
...
def backtest_contract(
    ...
    for i, candidate in enumerate(entries):
        ...
        for variant, threshold in VARIANTS.items():
            ...
            trade["breakout_body_atr"] = breakout_body_atr
            ...
            if candidate_close_ms < holdout_start_ms:
                ...
                pre_trade["breakout_body_atr"] = breakout_body_atr
...
def main() -> None:
    ...
    for name, threshold in VARIANTS.items():
        rows = full_trades[name]
        pre_rows = pre_holdout_trades[name]
        rows.sort(key=lambda item: (item["entry_time_ms"], item["symbol"]))
        pre_rows.sort(key=lambda item: (item["entry_time_ms"], item["symbol"]))
        holdout_rows = [trade for trade in rows if trade["entry_time_ms"] >= holdout_start_ms]
        enrich_times(rows)
        enrich_times(pre_rows)
        variants[name] = {
            "min_breakout_body_atr": threshold,
            "summary_180d": compact_summary(rows, starting_equity, risk_fraction),
            ...
            "risk_sensitivity_180d": risk_sensitivity(rows, starting_equity),
        }
    ...
    result = {
        ...
        "hypothesis": "Filter weak 4H breakout candles by body size relative to 4H ATR while keeping 15M entry and 4H exits unchanged.",
        "filter_definition": "abs(4H breakout close - open) / 4H ATR",
        "variants": variants,
        "coverage": {
    ...
    summary_path = out_dir / "4h_breakout_strength_summary.json"
    csv_path = out_dir / "4h_breakout_strength_trades.csv"
