...
PERIOD_DAYS = 180
HOLDOUT_DAYS = 30
VARIANTS = ("baseline", "wrong_side_close", "body_cross", "strong_body_cross")
...
def simulate_trade(
    ...
    result = {
        ...
        "entry_distance_atr4h": abs(signal.candle.close - signal.breakout_level) / float(signal.atr_monitor),
...
def variant_accepts(variant: str, signal: Any, entry_history: list[Candle]) -> bool:
    if variant == "baseline":
        return True
    ...
    if signal.direction == "up":
        ...
        body_cross = candidate.open <= level < candidate.close
        candle_range = candidate.high - candidate.low
        strong_close = candle_range > 0 and (candidate.close - candidate.low) / candle_range >= 0.65
    else:
        wrong_side_close = any(c.close >= level for c in previous)
        body_cross = candidate.open >= level > candidate.close
        candle_range = candidate.high - candidate.low
        strong_close = candle_range > 0 and (candidate.high - candidate.close) / candle_range >= 0.65

    if variant == "wrong_side_close":
        return wrong_side_close
    if variant == "body_cross":
        return body_cross
    if variant == "strong_body_cross":
        return body_cross and strong_close
...
def backtest_contract(
    ...
    full_trades: dict[str, list[dict[str, Any]]] = {name: [] for name in VARIANTS}
    pre_holdout_trades: dict[str, list[dict[str, Any]]] = {name: [] for name in VARIANTS}
    monitor_end = 0
    seen_setup_keys: dict[str, set[str]] = {name: set() for name in VARIANTS}
    ...
    for i, candidate in enumerate(entries):
        ...
        for variant in VARIANTS:
            if not variant_accepts(variant, signal, entry_history):
                continue
            if signal.key in seen_setup_keys[variant]:
                continue
            seen_setup_keys[variant].add(signal.key)
            full_trades[variant].append(simulate_trade(signal, entries, i, period_end_ms))
            if candidate_close_ms < holdout_start_ms:
                pre_holdout_trades[variant].append(
...
def main() -> None:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                for name in VARIANTS:
                    full_trades[name].extend(trades[name])
                    pre_holdout_trades[name].extend(pre_trades[name])
                coverage.append(cov)
                print(
    ...
    for name in VARIANTS:
        full_trades[name].sort(key=lambda item: (item["entry_time_ms"], item["symbol"]))
        pre_holdout_trades[name].sort(key=lambda item: (item["entry_time_ms"], item["symbol"]))
        enrich_times(full_trades[name])
        enrich_times(pre_holdout_trades[name])

    starting_equity = 200.0
    risk_fraction = settings.risk_per_trade
    variants = {}
    for name in VARIANTS:
        all_rows = full_trades[name]
        pre_rows = pre_holdout_trades[name]
        holdout_rows = [trade for trade in all_rows if trade["entry_time_ms"] >= holdout_start_ms]
        variants[name] = {
            "confirmation_variant": name,
            "summary_180d": compact_summary(all_rows, starting_equity, risk_fraction),
            "summary_pre_holdout_150d": compact_summary(pre_rows, starting_equity, risk_fraction),
            "summary_latest_30d": compact_summary(holdout_rows, starting_equity, risk_fraction),
            "risk_sensitivity_180d": risk_sensitivity(all_rows, starting_equity),
            "by_direction_180d": grouped_summary(all_rows, lambda trade: trade["direction"], starting_equity, risk_fraction),
        }
    ...
    result = {
        ...
        "hypothesis": "Keep 4H structure SL/TP fixed and require a clearer 15M reclaim/reject confirmation at the 4H roll level.",
        "variant_definitions": {
            "baseline": "Current production confirmation: touch within retest window + directional candle closing on trend side of roll level.",
            "wrong_side_close": "Baseline plus at least one prior 15M close on the invalid side of the roll level before reclaim/reject.",
            "body_cross": "Candidate 15M candle body itself opens on the invalid side and closes on the trend side of the roll level.",
            "strong_body_cross": "body_cross plus close in the trend-side 35% of the candidate candle range."
        },
        "variants": variants,
        "coverage": {
    ...
    summary_path = out_dir / "reclaim_reject_summary.json"
    csv_path = out_dir / "reclaim_reject_trades.csv"
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writeheader()
        for variant, rows in full_trades.items():
            for trade in rows:
                writer.writerow({field: (variant if field == "variant" else trade.get(field)) for field in fields})
