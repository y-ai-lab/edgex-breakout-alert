...
PERIOD_DAYS = 180
HOLDOUT_DAYS = 30
VARIANTS = ("baseline", "reclaim", "wick_rejection", "strong_reclaim")
...
def confirmation_variant_passes(signal: Any, candidate: Candle, variant: str) -> bool:
    """Compare stricter 15M confirmation rules without changing the 4H setup/SL/TP."""
    if variant == "baseline":
        return True

    level = float(signal.breakout_level)
    candle_range = max(candidate.high - candidate.low, 1e-12)
    body = abs(candidate.close - candidate.open)
    body_ratio = body / candle_range

    if signal.direction == "up":
        reclaim = candidate.low <= level and candidate.close > level
        lower_wick = min(candidate.open, candidate.close) - candidate.low
        rejection = reclaim and lower_wick >= body * 0.5
        close_location = (candidate.close - candidate.low) / candle_range
        strong = reclaim and body_ratio >= 0.5 and close_location >= 0.75
    else:
        reclaim = candidate.high >= level and candidate.close < level
        upper_wick = candidate.high - max(candidate.open, candidate.close)
        rejection = reclaim and upper_wick >= body * 0.5
        close_location = (candidate.high - candidate.close) / candle_range
        strong = reclaim and body_ratio >= 0.5 and close_location >= 0.75
...
def backtest_contract(
    ...
    for i, candidate in enumerate(entries):
        ...
        for variant in VARIANTS:
            if not confirmation_variant_passes(signal, candidate, variant):
                continue
...
def main() -> None:
    ...
    result = {
        ...
        "hypothesis": "Improve 15M reaction quality while keeping 4H setup, SL and TP fixed.",
        "variant_definitions": {
            "baseline": "Current rule: recent touch + directional candle closing beyond roll level.",
            "reclaim": "Current 15M candle itself trades through/touches the roll level and closes back beyond it.",
            "wick_rejection": "Reclaim plus rejection wick at least 0.5x candle body.",
            "strong_reclaim": "Reclaim plus body >= 50% of range and close in directional top/bottom 25%."
        },
        "variants": variants,
        "coverage": {
    ...
    summary_path = out_dir / "entry_confirmation_summary.json"
    csv_path = out_dir / "entry_confirmation_trades.csv"
